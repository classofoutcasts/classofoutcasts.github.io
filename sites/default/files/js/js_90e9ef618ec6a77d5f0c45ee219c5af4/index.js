...
;
$('document').ready(function() {
  // collect and normalize displayed terms along project-pictures generated by view:'project_detail'
  // block visibility code displays block only on pages: /project/projectname
  if($('#techniques #keywords ul') && $('#content-content .views-field-tid li')) {
    var seen = {};
    $('#content-content .views-field-tid li').each(function() {
    ...
    $('#content-content .views-field-tid li').each(function() {
      $(this).appendTo($('#techniques #keywords ul'));
    });
  }

  if($('#techniques #keywords ul') && $('#content-content .meta .terms .links li')) {
    var seen = {};
    $('#content-content .meta .terms .links li').each(function() {
      var term = $(this).find('a').text();
      var href = $(this).find('a').attr('href');
      if (seen[term]) {
        $(this).remove();
  ...
  if( $('#block-views-project_detail-block_1').length ) {

// calculate main content width according to browsers' width
      $('#content').width($(window).width() - 189);
      $('#block-views-project_detail-block_1').width($(window).width() - 370);

// calculate total horizontal width of all floated vieuws-rows
      var totalwidth= 0;
      $('.view-project-detail .view-content').find('img').each(function() {
          totalwidth += $(this).width() + 20;
      });
      $('.view-project-detail').width(totalwidth);

// insert prev & next button elements
      $('#content').after("<div id=\"scroller\"><span id=\"left\">&laquo;</span> &nbsp;&nbsp; <span id=\"right\">&raquo;</span></div>");

// trigger actions for prev & next buttons
      $('#right').click(function() {
          $("#block-views-project_detail-block_1").animate({scrollLeft: '+=510'}, 500);
      })
      $('#left').click(function() {
          $("#block-views-project_detail-block_1").animate({scrollLeft: '-=510'}, 500);
      })
...
;
$(document).ready(function(){
  $(".views-field-field-projectbutton-fid .imagecache-170x120crop").hover(function(){
    $(this).fadeTo("fast", 0); // This sets the opacity to 0% on hover
  },function(){
    $(this).fadeTo("fast", 1); // This sets the opacity back to 100% on mouseout
  });
});;
/*
 * Overscroll v1.3.0
...
//$('document').ready(function() {
//  $('.view-project-detail .view-content').after("<span>left</span> &nbsp;&nbsp; <span>right</span>");
//}
;
